...
# 				ball.bounce(x_bounce=True, y_bounce=True)
# 				return
# 			else: # dead center then only y co-ordinate will change
# 				ball.bounce(x_bounce=False, y_bounce=True)
# 				return
...
# 	for brick in bricks.bricks:
#     # if ball touches brick then -1 hitpoint from brick
# 		if ball.distance(brick) < 40: 
# 			brick.quantity -= 1
#         #if brick hp becomes 0 then remove brick from screen
# 			if brick.quantity == 0:
# 				brick.clear()
# 				brick.goto(3000, 3000)
# 				bricks.bricks.remove(brick)
